...
output_writer.open(output_file, cv2.VideoWriter_fourcc(
    'j', 'p', 'e', 'g'), frame_rate, (total_width, total_height), True)
...
while (True):
    ...
    if len(face_detect) > 0:
        ...
        detection_frame = frame[video_height//2:total_height -
                                video_height//2, video_width//2:total_width - video_width//2, :]

        ### Calculating the heart rate ###

        # Constructing the Gaussian pyramid
        # Broadcasting input array shape
        gaussian_video[buffer_index] = helper.create_gaussian_pyramid(
            detection_frame, levels+1)[levels]
        ...
        if buffer_index % bpm_frequency == 0:
            ...
            for buffer in range(buffer_size):
                fourier_transform_avg[buffer] = np.real(
                    fourier_transform[buffer]).mean()
        ...
        frame[video_height//2:total_height-video_height//2,
              video_width//2:total_width-video_width//2, :] = output_frame
        cv2.rectangle(frame, (video_width//2, video_height//2), (total_width -
                      video_width//2, total_height-video_height//2), box_color, box_weight)

        # Showing Heart Rate in BPM
        if i > bpm_buffer_size:
            cv2.putText(frame, "BPM: %d" % bpm_buffer.mean(),
                        HR_text_location, font_style, font_scale, font_color)
            HR.append(bpm_buffer.mean())

        # Loading the Heart rate
        else:
            cv2.putText(frame, "Measuring Heart Rate...",
                        init_text_location, font_style, font_scale, font_color)
    ...
    # When No face is detected
        ...
        # No face detected
        cv2.putText(frame, "No face detected", init_text_location,
                    font_style, font_scale, font_color)
